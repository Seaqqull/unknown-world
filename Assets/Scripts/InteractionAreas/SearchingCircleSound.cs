...
{
    public class SearchingCircleSound : SearchingArea
    {
        [SerializeField] [Range(0, ushort.MaxValue)] private float m_maxVolume = ushort.MaxValue;
        [SerializeField] [Range(0, ushort.MaxValue)] private float m_minVolume = 1.0f; // set max distance of tracing box to object center
        ...
        private bool ListenSound(UnknownWorld.Area.Target.TracingArea target, Vector3 positionOffset, Vector3 substraction)
        {
            float distance = Vector3.Distance(target.transform.position, positionOffset);
            Vector3 listener = positionOffset + (substraction.normalized * ((distance < m_data.Radius) ? distance : m_data.Radius));

            float noise = target.EmitSound(listener);            

            if ((noise < m_minVolume) ||
                (noise > m_maxVolume)) return true;
        ...
        public override bool IsTargetWithinArea(UnknownWorld.Area.Target.TracingArea[] target, BitArray affectionMask, params object[] list)
        {
            ...
            for (int i = 0; i < target.Length; i++)
            {
                ...
                if ((((1 << target[i].Collider.gameObject.layer) & m_data.TargetMask) == 0) ||
                    ...
                    ListenSound(target[i], positionWithOffset, vectorSubtraction))
                    continue;
